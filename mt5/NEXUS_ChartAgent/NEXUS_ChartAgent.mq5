...
#define NEXUS_CHART_AGENT_VERSION "0.6.5-chart-agent-inline-v2-diag5-scale-fallback"
#define NEXUS_CHART_VISUAL_PROFILE "approved-inline-level-v2"

string g_account = "";
string g_last_tag_error = "";
string g_last_scale_error = "";
int g_diag_timer_count = 0;
int g_diag_last_http_error = 0;
                           ...
                           const double &targets[])
{
   ...
   if(!ChartGetDouble(chart_id,CHART_PRICE_MIN,0,chart_min))
   {
      g_last_scale_error=
         "PRICE_MIN_READ_FAILED:err="+IntegerToString(GetLastError());
      return false;
   }

   ResetLastError();

   if(!ChartGetDouble(chart_id,CHART_PRICE_MAX,0,chart_max))
   {
      g_last_scale_error=
         "PRICE_MAX_READ_FAILED:err="+IntegerToString(GetLastError());
      return false;
   }

   if(chart_max<=chart_min)
   {
      double fallback_min=MathMin(entry,sl);
      double fallback_max=MathMax(entry,sl);
      ...
      for(int j=0;j<ArraySize(targets);j++)
      {
         ...
         fallback_min=MathMin(fallback_min,targets[j]);
         fallback_max=MathMax(fallback_max,targets[j]);
      }

      if(fallback_min<=0.0 || fallback_max<=fallback_min)
      {
         g_last_scale_error=
            "INVALID_FALLBACK_RANGE:min="+DoubleToString(fallback_min,2)+
            ",max="+DoubleToString(fallback_max,2);
         return false;
      }

      chart_min=fallback_min;
      chart_max=fallback_max;

      DiagTrace(
         "SCALE_RANGE_FALLBACK min="+DoubleToString(chart_min,2)+
         " max="+DoubleToString(chart_max,2)
      );
   }
   ...
   if(span<=0.0)
      span=MathMax(MathAbs(required_max)*0.01,1.0);
   ...
   {
      g_last_scale_error=
         "SCALEFIX_SET_FAILED:err="+IntegerToString(GetLastError());
      return false;
   }
   ...
   {
      g_last_scale_error=
         "FIXED_MIN_SET_FAILED:err="+IntegerToString(GetLastError())+
         ",value="+DoubleToString(fixed_min,2);
      return false;
   }
   ...
   {
      g_last_scale_error=
         "FIXED_MAX_SET_FAILED:err="+IntegerToString(GetLastError())+
         ",value="+DoubleToString(fixed_max,2);
      return false;
   }
   ...
   {
      g_last_scale_error=
         "VISIBLE_MIN_READ_FAILED:err="+IntegerToString(GetLastError());
      return false;
   }

   ResetLastError();

   if(!ChartGetDouble(chart_id,CHART_PRICE_MAX,0,visible_max))
   {
      g_last_scale_error=
         "VISIBLE_MAX_READ_FAILED:err="+IntegerToString(GetLastError());
      return false;
   }

   if(!(visible_min<required_min && visible_max>required_max))
   {
      g_last_scale_error=
         "RANGE_VERIFY_FAILED"+
         StringFormat(
            ":visible_min=%.2f,visible_max=%.2f,required_min=%.2f,required_max=%.2f,fixed_min=%.2f,fixed_max=%.2f",
            visible_min,
            visible_max,
            required_min,
            required_max,
                    ...
                    const int digits)
{
   ...
   {
      g_last_tag_error=
         "HEIGHT_FAILED:key="+key+
         ",err="+IntegerToString(GetLastError())+
         ",height="+IntegerToString((int)chart_height);
      return false;
   }

   int output_height=MathMax(1,InpScreenshotHeight);
   ...
   if(map_ok && y_bottom>y_top+1)
   {
      int top_margin=MathMax(
         0,
         y_top
      );

      int bottom_margin=MathMax(
         0,
         (int)chart_height-y_bottom
      );

      int output_top=top_margin;
      int output_bottom=output_height-bottom_margin;

      if(output_bottom>output_top+1)
      {
         double normalized=
            (double)(y-y_top)/
            (double)(y_bottom-y_top);

         center_y=(int)MathRound(
            (double)output_top+
            normalized*
            (double)(output_bottom-output_top)
         );
      }
      else
      {
   ...
   DiagTrace(
      "TAG_MAP key="+key+
      " live_y="+IntegerToString(y)+
      " out_y="+IntegerToString(center_y)+
      " live_h="+IntegerToString((int)chart_height)+
      " out_h="+IntegerToString(output_height)+
      " plot_top="+IntegerToString(y_top)+
      " plot_bottom="+IntegerToString(y_bottom)+
      " mapped="+IntegerToString(map_ok ? 1 : 0)
   );
   ...
   if(top<1 || top+height>output_height-1)
   {
      g_last_tag_error=
         "Y_BOUNDS:key="+key+
         ",live_y="+IntegerToString(y)+
         ",out_y="+IntegerToString(center_y)+
         ",top="+IntegerToString(top)+
         ",tag_h="+IntegerToString(height)+
         ",live_h="+IntegerToString((int)chart_height)+
         ",out_h="+IntegerToString(output_height)+
         ",price="+DoubleToString(price,digits);
      return false;
   ...
   {
      g_last_tag_error="CREATE_PRICE_BOX_FAILED:key="+key+",err="+IntegerToString(GetLastError());
      return false;
   }
   ...
   {
      g_last_tag_error="CREATE_NAME_BOX_FAILED:key="+key+",err="+IntegerToString(GetLastError());
      return false;
   }
                ...
                string &broker_symbol,string &image_b64,string &sha256,string &error_text)
{
   ...
   do
   {
      ...
      if(!FitTradeLevelsToChart(chart_id,entry,sl,targets))
      {
         error_text="TRADE_LEVEL_SCALE_FIT_FAILED:"+g_last_scale_error;
         break;
      }
      ...
      {
         error_text="ENTRY_TAG_DRAW_FAILED:"+g_last_tag_error;
         break;
      }
...
void DiagTrace(const string message)
{
   int handle=FileOpen(
      "NEXUS_ChartAgent_diag3.log",
...
void PollOnce()
{
   int token_len=StringLen(Trim(InpAdminToken));

   DiagTrace(
      "POLL_BEGIN token_len="+IntegerToString(token_len)
   );

   if(token_len<16)
   {
      DiagTrace(
         "POLL_SKIP_INVALID_TOKEN token_len="+IntegerToString(token_len)
      );
      return;
   }
   ...
   {
      DiagTrace(
         "POLL_HTTP_TRANSPORT_FAIL status="+IntegerToString(status)+
         " mql_error="+IntegerToString(g_diag_last_http_error)
      );
      return;
   }

   DiagTrace(
      "POLL_HTTP status="+IntegerToString(status)+
      " response_len="+IntegerToString(StringLen(response))
   );

   if(status!=200)
   {
      DiagTrace(
         "POLL_HTTP_REJECTED status="+IntegerToString(status)
      );

      Print(
         "[NEXUS ChartAgent] Poll HTTP=",
         status,
         " body=",
         response
      );
   ...
   if(StringFind(response,"\"job\":null")>=0)
   {
      DiagTrace("POLL_NO_JOB");
      return;
   }

   DiagTrace("POLL_JOB_AVAILABLE");
...
int OnInit()
{
   g_account=IntegerToString(
      (int)AccountInfoInteger(ACCOUNT_LOGIN)
   );

   g_diag_timer_count=0;

   int token_len=StringLen(
      Trim(InpAdminToken)
   );

   DiagTrace(
      "INIT_BEGIN account="+g_account+
      " version="+NEXUS_CHART_AGENT_VERSION+
      " api="+InpApiBaseUrl+
      " token_len="+IntegerToString(token_len)
   );

   if(token_len<16)
   {
      DiagTrace(
         "INIT_FAIL_INVALID_TOKEN token_len="+IntegerToString(token_len)
      );

      Print(
         "[NEXUS ChartAgent] InpAdminToken is not configured."
      );
   ...
   DiagTrace(
      "EVENT_SET_TIMER seconds="+IntegerToString(seconds)+
      " ok="+IntegerToString(timer_ok ? 1 : 0)+
      " err="+IntegerToString(timer_error)
   );

   if(!timer_ok)
   {
      DiagTrace(
         "INIT_FAIL_TIMER err="+IntegerToString(timer_error)
      );

      return INIT_FAILED;
   }

   DiagTrace("INIT_SUCCESS");

   Print(
      "[NEXUS ChartAgent] Started account=",
      g_account,
      " api=",
      InpApiBaseUrl,
      " interval=",
...
{
   DiagTrace(
      "DEINIT reason="+IntegerToString(reason)+
      " timer_count="+IntegerToString(g_diag_timer_count)
   );
...
void OnTimer()
{
   ...
   int timer_no=g_diag_timer_count;

   DiagTrace(
      "TIMER_BEGIN n="+IntegerToString(timer_no)
   );

   PollOnce();

   DiagTrace(
      "TIMER_END n="+IntegerToString(timer_no)
   );
}
