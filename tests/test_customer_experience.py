...
def test_main_menu_contains_required_customer_entries_and_folder_url():
    markup = cx.customer_main_menu("fa", is_admin=False, has_vip=False)
    buttons = _buttons(markup)
    labels = {b.text for b in buttons}
    assert "💎 خرید اشتراک" in labels
    assert "❓ سوالات متداول" in labels
    assert "🚪 ورود به نکسوس" in labels
    assert "🔒 کانال سیگنال VIP" in labels
    nexus = next(b for b in buttons if b.text == "🚪 ورود به نکسوس")
    assert nexus.url == "https://t.me/+UZgLQ-uYwn5jNGE0"


def test_vip_button_unlocks_when_subscription_is_active():
    locked = _buttons(cx.customer_main_menu("fa", is_admin=False, has_vip=False))
    unlocked = _buttons(cx.customer_main_menu("fa", is_admin=False, has_vip=True))
...
def test_faq_is_expandable_and_has_multiple_questions():
    markup = cx.faq_menu("fa")
    faq_callbacks = [b.callback_data for b in _buttons(markup) if b.callback_data and b.callback_data.startswith("faq:")]
