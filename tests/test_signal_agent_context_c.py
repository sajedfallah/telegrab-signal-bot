...
def candles(tf):
    z=datetime(2026,1,1,tzinfo=timezone.utc);s=SEC[tf]
    return [Candle("XAUUSD",tf,z+timedelta(seconds=s*i),10+i,12+i,9+i,11+i,10) for i in range(8)]
...
def fixture():
    c={tf:candles(tf) for tf in SEC};z=c["D1"][1].open_time
    e=[ev("D1",z,"MSS","BULLISH",12),ev("H4",c["H4"][2].open_time,"MSS","BULLISH",12),ev("H1",c["H1"][2].open_time,"MSS","BULLISH",12),ev("M15",c["M15"][2].open_time,"MSS","BEARISH",12),ev("M5",c["M5"][2].open_time,"MSS","BEARISH",12)]
    e += [ev("D1",z,"DAILY_QUADRANT","UPPER",lo=14,hi=20,ref=17,meta={"levels":{"25":15.5,"50":17,"75":18.5}}),ev("H1",c["H1"][3].open_time,"LIQUIDITY_POOL","BUY_SIDE",ref=25),ev("H1",c["H1"][4].open_time,"LIQUIDITY_POOL","SELL_SIDE",ref=10),ev("M15",c["M15"][4].open_time,"LIQUIDITY_SWEEP","BULLISH",ref=10),ev("H1",c["H1"][3].open_time,"FVG","BULLISH",lo=15,hi=16)]
    return c,e
def test_context_mtf_bias_location_quadrant_liquidity_poi_session():
    c,e=fixture();a=c["M5"][-1].open_time+timedelta(seconds=300);s=ContextEngine().build("XAUUSD",c,e,a)
    assert s.htf_bias=="BULLISH";assert s.structure_alignment=="HTF_ALIGNED_LTF_COUNTERTREND"
    assert set(s.timeframe_bias)==set(SEC);assert s.dealing_range and s.price_location in ("PREMIUM","DISCOUNT","EQUILIBRIUM")
    assert s.daily_quadrant["priority_levels"]==["50","75"];assert s.liquidity["nearest_buy_side"];assert s.recent_sweeps;assert s.active_pois
...
def test_deterministic_key_and_states():
    c,e=fixture();a=c["M5"][-1].open_time+timedelta(seconds=300);x=ContextEngine().build("XAUUSD",c,e,a);y=ContextEngine().build("XAUUSD",c,list(reversed(e)),a)
