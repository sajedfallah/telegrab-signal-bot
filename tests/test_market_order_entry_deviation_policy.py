...
def test_market_signal_rejects_literal_zero_max_entry_deviation_pct():
    # A MARKET order's fill price always differs from the quoted entry by at
    # least a fraction of a pip by the time the EA executes. A literal 0
    # therefore can never be satisfied and must be rejected at the API
    # boundary rather than silently accepted and forwarded to the EA, where
    # it causes every MARKET signal to be rejected with
    # "entry deviation X% exceeds 0.00000%".
...
def test_market_signal_omitted_deviation_is_none_not_zero():
    # Omitting the field (the path the EA's own ISSUE SIGNAL panel always
    # takes) must resolve to None so the EA falls back to its safe
    # InpDefaultMaxEntryDeviationPct, not to a literal 0.
