...
class CandleSeries(BaseModel):
    ...
    candles: list[CandlePoint] = Field(
        min_length=1,
        max_length=_MAX_CANDLES_PER_SERIES,
    )
...
class MarketFeedRequest(BaseModel):
    ...
    ea_version: str | None = Field(
        default=None,
        max_length=32,
    )
    quotes: list[MarketQuote] = Field(
        default_factory=list,
        max_length=_MAX_SERIES,
    )
    series: list[CandleSeries] = Field(
        min_length=1,
        max_length=_MAX_SERIES,
    )
...
def _admin_auth(
    ...
    if str(admin_mode or "").strip().lower() not in {
        "1",
        "true",
        "yes",
        "on",
        ...
        raise HTTPException(
            status_code=403,
            detail="admin mode is required for broker candle feed",
        )

    try:
        return authorize_admin_mt5(
            account,
            admin_token,
        )
...
def ingest_market_candles(
    ...
    if account != str(req.account_number).strip():
        raise HTTPException(
            status_code=409,
            detail="MT5 account header/body mismatch",
        )
        ...
        for quote in req.quotes:
            ...
            con.execute(
                ...
                DO UPDATE SET
                    ...
                    quote_time_ms=excluded.quote_time_ms,
