...
def init_db() -> None:
    with conn() as con:
        ...
        con.executescript(
            ...
            CREATE INDEX IF NOT EXISTS idx_chart_capture_status
                ON signal_chart_capture_jobs(status,next_attempt_at,id);
            CREATE INDEX IF NOT EXISTS idx_chart_capture_signal
                ON signal_chart_capture_jobs(signal_id,id);
            CREATE INDEX IF NOT EXISTS idx_chart_capture_requested
                ON signal_chart_capture_jobs(requested_at);
        ...
        authority_migrations = {
            ...
            "publication_stage": "ALTER TABLE signals ADD COLUMN publication_stage TEXT",
        }
...
def ensure_default_plans(defaults: dict[str, dict[str, object]]) -> None:
    ...
    with conn() as con:
        ...
        catalog_fix = {"AEX1M": "5", "AEX3M": "14", "AEX6M": "27", "AEX12M": "49"}
...
def create_chart_capture_job(signal_id: int, requested_by: str, *, ttl_seconds: int = 300) -> sqlite3.Row:
    ...
    with conn() as con:
        existing = con.execute(
            "SELECT * FROM signal_chart_capture_jobs WHERE signal_id=? AND status IN "
            "('PENDING','CLAIMED','CAPTURING','UPLOADED') ORDER BY id DESC LIMIT 1",
            (int(signal_id),),
        ).fetchone()
        if existing:
            return existing
...
def get_signal_chart_capture_job(signal_id: int) -> sqlite3.Row | None:
    with conn() as con:
        return con.execute(
            "SELECT * FROM signal_chart_capture_jobs WHERE signal_id=? ORDER BY id DESC LIMIT 1", (int(signal_id),)
        ).fetchone()
...
def claim_next_chart_capture_job(account_number: str) -> sqlite3.Row | None:
    ...
    with conn() as con:
        ...
        con.execute(
            "UPDATE signal_chart_capture_jobs SET status='EXPIRED',failed_at=?,error_text='job expired',updated_at=? "
            "WHERE status='PENDING' AND expires_at<?", (now_s, now_s, now_s),
        )
        row = con.execute(
        ...
        cur = con.execute(
            """UPDATE signal_chart_capture_jobs
               ...
               WHERE id=? AND status='PENDING'""", (account, now_s, now_s, int(row["id"])),
        )
        if cur.rowcount != 1:
            return None
    ...
    with conn() as con:
        ...
        con.execute(
            """UPDATE signal_chart_capture_jobs SET status='UPLOADED',completed_at=?,image_path=?,image_sha256=?,
               error_text=NULL,updated_at=? WHERE id=?""", (now, str(image_path), str(image_sha256), now, int(job_id)),
        )
        return con.execute("SELECT * FROM signal_chart_capture_jobs WHERE id=?", (int(job_id),)).fetchone(), True
...
def fail_chart_capture_job(job_id: int, account_number: str, error_text: str) -> sqlite3.Row:
    ...
    with conn() as con:
        ...
        con.execute(
            ...
            (status, now_s if terminal else None, str(error_text)[:1000],
             (now + timedelta(seconds=delay)).isoformat(), now_s, int(job_id)),
        )
        return con.execute("SELECT * FROM signal_chart_capture_jobs WHERE id=?", (int(job_id),)).fetchone()
...
def mark_chart_capture_job_completed(job_id: int) -> None:
    with conn() as con:
        con.execute(
            "UPDATE signal_chart_capture_jobs SET status='COMPLETED',updated_at=? WHERE id=? AND status='UPLOADED'",
            (now_iso(), int(job_id)),
        )
...
def create_autotrade_command(signal_id: int, command: str, payload: dict | None = None, *, actor_type: str = "MT5_ADMIN", actor_id: str | int | None = None, account_number: str | None = None) -> int:
    ...
    if "issuer_type" in signal.keys() and str(signal["issuer_type"]).upper() not in {"MT5_ADMIN", "WEB_ADMIN"}:
        raise ValueError("only MT5_ADMIN or WEB_ADMIN signals accept v0.6 admin commands")
    now = now_iso()
    with conn() as con:
