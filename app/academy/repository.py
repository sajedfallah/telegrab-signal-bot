    ...
    with db.conn() as con:
        con.execute(
            ...
            ON CONFLICT(scheduled_date) DO UPDATE SET
                ...
                exercise_prompt=excluded.exercise_prompt,exercise_options_json=excluded.exercise_options_json,
...
def mark_published(day: str, message_id: int) -> None:
    now = _now()
    with db.conn() as con:
        con.execute("UPDATE academy_lessons SET status='published',telegram_message_id=?,published_at=?,updated_at=? WHERE scheduled_date=?",
...
def record_answer(lesson_id: int, user_id: int, selected_option: int, is_correct: bool) -> None:
    ensure_schema()
    with db.conn() as con:
        con.execute("""INSERT INTO academy_answers(lesson_id,user_id,selected_option,is_correct,created_at)
                       VALUES(?,?,?,?,?)
...
def stats_for_lesson(lesson_id: int) -> tuple[int, int]:
    ensure_schema()
    with db.conn() as con:
        row = con.execute("SELECT COUNT(*) AS total, COALESCE(SUM(is_correct),0) AS correct FROM academy_answers WHERE lesson_id=?",
                          (int(lesson_id),)).fetchone()
...
def recent_lessons(limit: int = 10):
    ensure_schema()
    with db.conn() as con:
        return con.execute("SELECT * FROM academy_lessons ORDER BY scheduled_date DESC LIMIT ?", (max(1, min(30, int(limit))),)).fetchall()
