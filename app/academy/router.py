...
async def academy_today(message: Message) -> None:
    ...
    await message.answer(f"📘 درس {row['lesson_number']}\n{row['title']}\nوضعیت: {row['status']}\nکلیدواژه: {row['primary_keyword']}\nپاسخ تمرین: {total} | تکمیل موفق: {pct}%")
...
async def academy_answer(callback: CallbackQuery) -> None:
    try:
        ...
        text = "عالیه 👌 تمرین امروز ثبت شد." if choice == 0 else ("خوبه؛ یک بار دیگه روی نمودار واقعی تمرینش کن." if choice == 1 else "این درس رو دوباره با مثال مرور کن.")
