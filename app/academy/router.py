...
async def academy_preview(message: Message) -> None:
    if not _is_admin(message):
        return
    try:
        day = _parse_day(_arg(message))
        ok = await _agent.preview(message.bot, day)
        await message.answer("✅ پیش‌نمایش آماده شد." if ok else "❌ پیش‌نمایش ساخته نشد.")
    except Exception as exc:
        repository.record_failure((_parse_day(None)).isoformat(), "manual_preview", str(exc))
        await message.answer(f"❌ خطا در ساخت پیش‌نمایش: {str(exc)[:300]}")
...
async def academy_approve(message: Message) -> None:
    ...
    try:
        day = _parse_day(_arg(message))
        ok = await _agent.publish(message.bot, day)
        await message.answer("✅ درس در Academy منتشر شد." if ok else "❌ انتشار انجام نشد.")
    except Exception as exc:
        await message.answer(f"❌ خطای انتشار: {str(exc)[:300]}")
...
async def academy_cancel(message: Message) -> None:
    ...
    try:
        day = _parse_day(_arg(message))
        repository.mark_cancelled(day.isoformat())
        await message.answer(f"⛔ درس {day.isoformat()} لغو شد.")
    except Exception:
        await message.answer("فرمت تاریخ: YYYY-MM-DD یا today")
...
async def academy_today(message: Message) -> None:
    if not _is_admin(message):
        return
    day = _parse_day(None)
    row = repository.get_by_day(day)
    if row is None:
        await message.answer("ℹ️ برای امروز هنوز درسی ساخته نشده است.")
        return
    total, correct = repository.stats_for_lesson(int(row["id"]))
    pct = round((correct / total) * 100) if total else 0
    await message.answer(
        f"📘 درس {row['lesson_number']}\n"
        f"{row['title']}\n"
        f"وضعیت: {row['status']}\n"
        f"کلیدواژه: {row['primary_keyword']}\n"
        f"پاسخ تمرین: {total} | تکمیل موفق: {pct}%"
    )
...
async def academy_stats(message: Message) -> None:
    ...
    if not rows:
        await message.answer("هنوز آماری ثبت نشده است.")
        return
    lines = ["📊 آمار ۱۰ درس اخیر"]
    for row in rows:
        total, correct = repository.stats_for_lesson(int(row["id"]))
        pct = round((correct / total) * 100) if total else 0
        lines.append(f"• درس {row['lesson_number']} | {row['status']} | مشارکت {total} | موفق {pct}%")
    await message.answer("\n".join(lines))
...
async def academy_answer(callback: CallbackQuery) -> None:
    try:
        _, lesson_raw, choice_raw = str(callback.data).split(":", 2)
        lesson_id = int(lesson_raw)
        choice = int(choice_raw)
        row = repository.get_by_id(lesson_id)
        if row is None or callback.from_user is None:
            await callback.answer("این تمرین در دسترس نیست.", show_alert=True)
            return
        correct = choice == int(row["correct_option"] or 0)
        repository.record_answer(lesson_id, int(callback.from_user.id), choice, correct)
        if choice == 0:
            text = "عالیه 👌 تمرین امروز ثبت شد. فردا یک قدم جلوتر می‌ریم."
        elif choice == 1:
            text = "خوبه؛ یک بار دیگه روی نمودار واقعی تمرینش کن."
        else:
            text = "اشکالی نداره؛ این درس رو ذخیره کن و دوباره با مثال مرورش کن."
        await callback.answer(text, show_alert=True)
    except Exception:
        await callback.answer("ثبت پاسخ انجام نشد. دوباره امتحان کن.", show_alert=True)
