...
class StickerStore:
    ...
    def get_sticker(self, day: date | str) -> sqlite3.Row | None:
        key = self._key(day)
        with self._connect() as con:
    ...
    def delete_sticker(self, day: date | str) -> bool:
        key = self._key(day)
        with self._connect() as con:
            cur = con.execute("DELETE FROM daily_stickers WHERE date_key=?", (key,))
    ...
    def was_delivered(self, day: date | str, target: int | str) -> bool:
        ...
        with self._connect() as con:
            row = con.execute(
                "SELECT 1 FROM daily_sticker_deliveries WHERE date_key=? AND target=?",
                (key, str(target)),
            ).fetchone()
    ...
    def mark_delivered(self, day: date | str, target: int | str, message_id: int | None) -> None:
        ...
        with self._connect() as con:
            con.execute(
                """
                INSERT INTO daily_sticker_deliveries(date_key, target, message_id, sent_at)
                VALUES(?,?,?,?)
                ON CONFLICT(date_key, target) DO UPDATE SET
