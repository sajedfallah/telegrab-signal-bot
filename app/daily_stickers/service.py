...
def target_topic_id() -> int | None:
    raw = os.getenv("DAILY_STICKER_TOPIC_ID", "").strip()
    ...
    try:
        ...
        raise RuntimeError(
            "DAILY_STICKER_TOPIC_ID / PUBLIC_CONTENT_TOPIC_ID must be an integer"
        ) from exc
...
async def send_for_date(
    ...
    *,
    ...
    msg = await bot.send_sticker(**send_kwargs)
