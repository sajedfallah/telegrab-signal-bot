...
def format_hourly_analysis(record:Mapping[str,object])->str:
    ...
    fast=record.get("fast_scalp")
    if analysis_tf=="M1" and isinstance(fast,Mapping):
        state=str(fast.get("state") or "SEARCHING")
        state_fa={"SEARCHING":"در حال جست‌وجوی موقعیت","APPROACHING_POI":"قیمت در حال نزدیک‌شدن به POI","IN_POI":"قیمت داخل POI؛ تریگر M1 مسلح","LIQUIDITY_SWEPT":"نقدینگی M1 جمع شده؛ منتظر MSS","MSS_CONFIRMED":"MSS یک‌دقیقه تأیید شده؛ منتظر FVG","RETEST":"FVG یک‌دقیقه تشکیل شده؛ منتظر Retest","ENTRY_READY":"تریگر Fast Scalp M1 کامل شده","INVALIDATED":"ستاپ نامعتبر شده"}.get(state,state)
        lines.extend(["","⚡ Fast Scalp M1:",f"• وضعیت: {state_fa}"])
        poi=fast.get("poi"); zone=fast.get("entry_zone"); anchor=fast.get("sweep_anchor")
        if poi: lines.append(f"• POI: {float(poi[0]):g} تا {float(poi[1]):g}")
        if anchor is not None: lines.append(f"• Sweep Anchor: {float(anchor):g}")
        if zone: lines.append(f"• M1 FVG / Entry Zone: {float(zone[0]):g} تا {float(zone[1]):g}")
        blocks=list(fast.get("blocks") or [])
        if blocks: lines.append("• مرحله بعد: "+", ".join(str(x) for x in blocks))
