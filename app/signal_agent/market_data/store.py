...
class MarketDataStore:
    ...
    def load_state(self,symbol:str)->dict:
        with self._con() as c:
            q=c.execute("SELECT * FROM signal_agent_quotes WHERE symbol=?",(symbol,)).fetchone()
            candles={}
            for tf in ("M5","M15","H1","H4","D1"):
                r=c.execute("SELECT * FROM signal_agent_candles WHERE symbol=? AND timeframe=? ORDER BY open_time DESC LIMIT 1",(symbol,tf)).fetchone()
                if r: candles[tf]=dict(r)
        return {"quote":dict(q) if q else None,"candles":candles}
